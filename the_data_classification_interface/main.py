...
class DataClassificationApp(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        self.colors=[QColor(220, 240, 255),QColor(230, 255, 230),QColor(240, 230, 255),QColor(255, 235, 220)]
    ...
    def load_conversation(self,diss_ID):
        ...
        for i in range(0,len(data),4):
            ...
            self.conversation_space.addWidget(ConversationPage(messages,values,colors))
    ...
    def load_discussions(self):
        ...
        labels=[f"conv{ids[i]}: {lengths[i]} Lines | done {done[i]}/{lengths[i]}" for i in range(len(ids))]
