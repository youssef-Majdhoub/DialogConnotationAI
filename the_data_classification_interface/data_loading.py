...
class DataCleaner:
    ...
    def clean(self, path):
        ...
        re = pd.read_table(
            ...
            names=cols,
            on_bad_lines=self.bad_line_function,
        )
        return re.dropna()
    ...
    def integrity_check(self, discussions_frame, lines_frame):
        ...
        for i in range(len(discussions_frame)):
            lines = set(
                ast.literal_eval(
                    discussions_frame.iloc[i]["utterance_ids"].replace(" ", ",")
                )
            )
    ...
    def save_cleaned(self, repair_mode=False):
        ...
        for i in range(len(self.paths)):
            self.index = i
            if not (repair_mode) and os.path.exists(clean_paths[i]):
                continue
            re.append(self.clean(self.paths[i]))
        if len(re) == 2:
            diss, line = re
            diss, line = self.integrity_check(diss, line)
            diss.to_csv(clean_paths[0], sep="\t", index=False, header=False)
            line.to_csv(clean_paths[1], sep="\t", index=False, header=False)
        elif len(re) == 1:
            print(
                "integrity not checked please activate repair mode for safer data handling"
            )
        return clean_paths
...
class DataProvider:
    ...
    def create_data_set(self):
        conversations = pd.read_csv(
            ...
            sep="\t",
            header=None,
            names=["per1", "per2", "movie_id", "utterance_ids"],
        )
        lines = pd.read_csv(
            self.lines_path,
            sep="\t",
        ...
        for i in tqdm(range(len(conversations)), desc="Creating conversations"):
            ...
            disscussion_df.to_csv(
                f"./data_set/conversations/conversation_{i+1}.tsv",
                sep="\t",
                index=False,
            )
            disscussion_stat_df = self.create_disscussion_stat(i, len(disscussion_df))
            disscussion_stat_df.to_csv(
                f"./data_set/stats/stat_{i+1}.tsv", sep="\t", index=False
            )
    ...
    def create_conversation(self, conversations, lines, conversation_id):
        ...
        for i in range(len(conversation_lines)):
            ...
            disscussion["character_id"].append(
                conversation_lines.iloc[i]["character_id"]
            )
    ...
    def data_set_append(self):
        ...
        for i in tqdm(range(starting_index, len(conversations))):
            ...
            self.data_stat.loc[len(self.data_stat)] = {
                "id": i + 1,
                "disscussion_length": len(disscussion_df),
            }
        self.data_stat.to_csv("./data_set/data_stat.tsv", sep="\t", index=False)
    ...
    def repair_mode_from_raw_data(self):
        ...
        try:
            ...
            self.data_stat = pd.DataFrame(
                {"id": [], "disscussion_length": [], "is_complete": []}
            )
        ...
        for i in tqdm(range(len(conversations)), desc="Repairing dataset"):
            ...
            if not (i + 1) in ids:
                if stat:
                    stat_df = pd.read_table(
                        f"./data_set/stats/stat_{i+1}.tsv", sep="\t"
                    )
                new["id"].append(i + 1)
                length = len(stat_df)
    ...
    def get_conversation(self, conversation_id, include_stat=False):
        disscussion_df = pd.read_csv(
            f"./data_set/conversations/conversation_{conversation_id}.tsv",
            sep="\t",
        )
        if include_stat:
            disscussion_stat_df = pd.read_csv(
                f"./data_set/stats/stat_{conversation_id}.tsv",
                sep="\t",
            )
            disscussion_df = pd.merge(
                disscussion_df,
                disscussion_stat_df,
                left_on=["id", "lines_index"],
                right_on=["id", "lines_index"],
            )
        return disscussion_df

    def get_disscusion_stats(self, conversation_id):
        disscussion_stat_df = pd.read_csv(
            f"./data_set/stats/stat_{conversation_id}.tsv", sep="\t"
        )
        return disscussion_stat_df
    ...
    def update_connotation(self, conversation_id, line_index, connotation):
        ...
        disscussion_stat_df.loc[
            disscussion_stat_df["lines_index"] == line_index, "connotation"
        ] = connotation
        disscussion_stat_df.to_csv(
            f"./data_set/stats/stat_{conversation_id}.tsv", sep="\t", index=False
        )
        is_completed = (
            len(disscussion_stat_df) - disscussion_stat_df["connotation"].isna().sum()
        )
        self.data_stat.loc[self.data_stat["id"] == conversation_id, "is_complete"] = (
            is_completed
        )
